...
<form method="POST" action="{{ route('admin.departments.store') }}">
    ...
    <div class="form-group">
        ...
        <input type="number" id="complaints_assigned" name="complaints_assigned" value="{{ old('complaints_assigned', 0) }}" class="form-control" min="0" required>
    </div>

    <div class="form-group">
        <label for="staff_count" class="form-label">Staff Count</label>
        <input type="number" id="staff_count" name="staff_count" value="{{ old('staff_count', 0) }}" class="form-control" min="0" required>
    </div>

    <div class="form-group">
        <label for="actions" class="form-label">Actions</label>
        <textarea id="actions" name="actions" class="form-control" rows="4">{{ old('actions') }}</textarea>
    </div>
